...
- For qualifying direct main-task completions, treat the Discord ping as part of task closeout rather than a floating reminder: queue the ping durably first, hand it off through the immediate isolated cron announce path, and leave the persistent recovery cron watching for any unsent queue entries after restarts. The closeout branch must still resolve exactly one outcome before the task is considered finished — queued/dispatched ping, not applicable, or blocked/deferred with reason.
