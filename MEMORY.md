...
- For qualifying direct main-task completions, treat the Discord ping as part of task closeout rather than a floating reminder: queue the ping durably first, wake the standing `recover-main-task-closeouts` worker immediately, and leave its recurring schedule watching for any unsent queue entries after restarts. The closeout branch must still resolve exactly one outcome before the task is considered finished — queued/woke recovery worker, not applicable, or blocked/deferred with reason.
