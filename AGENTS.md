...
**Main-completion Discord best-effort ping:** when main (this agent in direct chat with haolun) fully completes a task assigned directly by haolun, try one brief best-effort completion ping through `scripts/main_task_closeout.py` and the configured Discord target in `mission-control/data/notifications.json`. Do not block task closure on delivery success or maintain a standing recovery poller for missed pings; if a ping is missed, leave it missed. Keep the ping to one short line, mention haolun, include an extremely brief analogy, and say the full report is in the control UI. Do not send these for subagent-only work, partial progress, or tasks not directly assigned by haolun. `#control-center` remains a manual fallback reference, not an automatic retry target.
