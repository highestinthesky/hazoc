...
**Main-completion Discord closeout gate:** when main (this agent in direct chat with haolun) fully completes a task assigned directly by haolun, do not treat the task as fully closed until the Discord completion-update branch is durably resolved through `scripts/main_task_closeout.py`: queue the notification first, then create the immediate isolated cron announce job, and rely on the persistent recovery cron if a restart interrupts the handoff. The valid resolved branches are: (1) queued/dispatched ping, (2) not applicable, or (3) blocked/deferred with reason. If applicable, send one brief completion ping through the configured Discord target in `mission-control/data/notifications.json`. Keep it to one short line, mention haolun, include an extremely brief analogy, and say the full report is in the control UI. Do not send these for subagent-only work, partial progress, or tasks not directly assigned by haolun. If `#main_updates` does not exist/bind yet, use the configured fallback Discord channel.
