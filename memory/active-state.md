...
- Main-only Discord completion updates are now configured locally through `mission-control/data/notifications.json`, using a tested one-shot cron announce delivery path to the current fallback Discord channel until the preferred `hzwp / #main_updates` channel exists and is bound.
