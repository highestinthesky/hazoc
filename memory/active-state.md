...
- Main-only Discord completion updates are now configured locally through `mission-control/data/notifications.json`, using a tested one-shot cron announce delivery path directly to `hzwp / #main_updates` (`1487645227421536327`), with `#protected-space` kept only as fallback.
