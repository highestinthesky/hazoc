...
- Main-only Discord completion updates are now configured locally through `mission-control/data/notifications.json`, with a fallback route to the existing bound Discord channel until the preferred `hzwp / #main_updates` channel exists and is bound.
